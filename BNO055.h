...
public:
	BNO055() { Wire.begin(); }
	//if orientation is > 7 you need to call orientation yourself before begin. Default orientation is P1 otherwise.
	/*
	The paramater list has the following description
	1: A POWER_MODE enum selection - See BNODEFS.h
	2: An OPERATION_MODE enum selection - See BNODEFS.h
	3: A PLACEMENT enum slection - See BNODEFS.h/datasheet
	4: Boolean value indicating whether calibrations should be loaded or not
	5: The pin number connected to Reset line, 255 for NC
	6: The pin number connected to address line, 255 for NC
	7: Should the address line be high/1?

	Only the first 2 would be required, or you can pass no parameters and no setup will be done, just checks that MPU is OK
	*/
  ...
  //BNO055 register read/write functions
  ...
  Android pitch: 180 - (-)180 clockwise decrease
  Roll: (-)90 - 90 increase with +inclination
  Heading/Yaw: 0 - 360* Clockwise increases
  */
  
	//Set measurement units for data outputs, 8 bit
...
private:
		...
		bool running = false; //internal variable for config state, true means NOT config state
